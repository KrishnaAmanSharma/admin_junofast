...
class _AvailableOrdersPageState extends State<AvailableOrdersPage> {
  ...
  Future<void> _refreshOrders() async {
    ...
    await ordersController.loadAvailableOrders(authService);
  }

  List<OrderModel> get filteredOrders {
    final ordersController = Provider.of<OrdersController>(context, listen: false);
    List<OrderModel> orders = ordersController.availableOrders;
    ...
    switch (sortBy) {
      case 'price_high':
        orders.sort((a, b) => (b.approxPrice ?? 0).compareTo(a.approxPrice ?? 0));
        break;
      case 'price_low':
        orders.sort((a, b) => (a.approxPrice ?? 0).compareTo(b.approxPrice ?? 0));
        break;
      case 'distance':
  ...
  Widget build(BuildContext context) {
    return Consumer2<OrdersController, AuthService>(
      builder: (context, ordersController, authService, child) {
        return Scaffold(
  ...
  void _showAcceptOrderDialog(OrderModel order) {
    ...
    priceController.text = order.approxPrice?.toStringAsFixed(2) ?? '';

    showDialog(
      context: context,
      builder: (context) => Consumer2<OrdersController, AuthService>(
        builder: (context, controller, authService, child) => AlertDialog(
            ...
            ElevatedButton(
              onPressed: controller.isAcceptingOrder 
                  ...
                  : () async {
                      ...
                      final success = await controller.acceptOrder(order.id, proposedPrice, authService);
                      if (success) {
