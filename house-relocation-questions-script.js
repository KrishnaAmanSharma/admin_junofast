...
async function setupHouseRelocationQuestions() {
  try {
    ...
    // Get or create House Relocation service type
    let { data: serviceType, error: serviceTypeError } = await supabase
      .from('service_types')
      .select('*')
      .ilike('name', '%house%')
      .single();
    ...
    if (serviceTypeError || !serviceType) {
      console.log('Creating House Relocation service type...');
    ...
    console.log(`📋 Service Type: ${serviceType.name} (${serviceTypeId})`);
    ...
    const existingOrders = new Set(existingQuestions?.map(q => q.display_order) || []);
    console.log(`Found ${existingQuestions?.length || 0} existing questions`);

    // Complete list of House Relocation Questions
    const questions = [
      {
        ...
        display_order: 1,
        options: {
          choices: ["Apartment/Flat", "Independent House/Villa", "Duplex", "Studio Apartment", "Penthouse", "Row House", "Other"]
        }
      },
      {
        ...
        display_order: 2,
        options: {
          choices: ["Studio (0 bedroom)", "1 BHK", "2 BHK", "3 BHK", "4 BHK", "5+ BHK"]
        }
      },
      {
        ...
        is_required: true,
        display_order: 3,
        options: { min: 100, max: 10000, placeholder: "e.g., 1200" }
      },
      {
        ...
        display_order: 4,
        options: {
          choices: ["Ground Floor", "1st Floor", "2nd Floor", "3rd Floor", "4th Floor", "5th Floor", "6th Floor", "7th Floor", "8th Floor", "9th Floor", "10+ Floor"]
        }
      },
      {
      ...
      },
      {
        ...
        display_order: 13,
        options: {
          choices: ["1-10 boxes", "11-25 boxes", "26-50 boxes", "51-75 boxes", "76-100 boxes", "100+ boxes"]
        }
      },
      {
      ...
      },
      {
        ...
        display_order: 16,
        options: {
          choices: ["Small Tempo (for studio/1BHK)", "Large Tempo/Mini Truck (for 2BHK)", "Container Truck (for 3BHK+)", "Multiple Vehicles (large house)", "Let movers decide"]
        }
      },
      {
    ...
    const newQuestions = questions.filter(q => !existingOrders.has(q.display_order));
    ...
    console.log('\n🎉 House Relocation Questions Setup Complete!');
    console.log(`📊 Total Questions: ${questions.length}`);
    console.log('📋 Categories: Property Info, Furniture, Electronics, Kitchen, Special Items, Services, Transport, Requirements');
